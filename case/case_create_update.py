...
xtype = EM7_VALUES["%1"]  # event xtype (0 = Org, 1 = device)

# event xtype (device 1, org 0) will control which start data query to use
if xtype == "0":
    # handle non-device events (eg: Org)
    notes_list.append("Organization level event, no Device, CI/Asset or location data available")
    ...
    #  reset "_fileter" to use the root device id from the DCM tree
...
# etype = EM7_VALUES["%3"]
# existing_case_number = EM7_VALUES["%_ext_ticket_ref"]
# severity = EM7_VALUES["%s"]


case_template = fidelus.silo.get_case_template(etype)

# common regardless of event xtype
# company = start_data["company"]
# snow_company = start_data["billing_id"]
# etype = start_data["etype"]
# etype_yname = start_data["yname"]
